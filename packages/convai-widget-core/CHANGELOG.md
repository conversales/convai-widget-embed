...
- Refresh the compact chat UI with a taller sheet, a branded Conversales header, and updated footer branding.

## 0.12.5

### Patch Changes

- Register both `conversales-convai` and the legacy `elevenlabs-convai` custom element names so the published embed bundle stays compatible while the package rename rolls out.
